...
class OrderConversion extends Card
{
    ...
    public function handle(Request $request)
    {
        ...
        $access_log = $access_log->whereBetween('created_at', [$dateRange['start'], $dateRange['end']])->select('ip')->groupBy('ip')->get();

        $access_count = $access_log->count();

        $order_model = AccessLogServiceProvider::setting('order_model');
        $order_count = app($order_model)->whereBetween('created_at', [$dateRange['start'], $dateRange['end']])->count();
